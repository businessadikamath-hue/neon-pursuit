...
The Ultra balance pass was checked after the weather update: Ultra keeps 4K PBR map selection and the enhanced atmosphere tier, while micro aggregate shadow casting is reserved for Ultra+ and dynamic shadows are refreshed at 24 Hz. The patrol presentation no longer leaves a rear-view inset open for the full pursuit. The live cop-car picture-in-picture starts with the alert, reports roughly 2 seconds remaining on activation, and is hidden after 2.3 seconds while the chase state remains active; the forward camera continues rendering throughout.
