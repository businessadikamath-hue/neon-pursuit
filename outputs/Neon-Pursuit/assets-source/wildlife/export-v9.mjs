...
import {buildWildlifeModel,buildBirdModel,createWildlifeMaterial} from '../engine-v9/wildlifeArt.js';
import {buildGroomedFur} from '../engine-v9/wildlifeDetail8.js';
...
fs.writeFileSync('work/wildlife-assets-v9/assembled-v9.json.gz',zlib.gzipSync(JSON.stringify(data)));console.log(JSON.stringify({subjects:data.subjects.map(s=>s.name),bytes:fs.statSync('work/wildlife-assets-v8/assembled-gallery.json.gz').size}));
