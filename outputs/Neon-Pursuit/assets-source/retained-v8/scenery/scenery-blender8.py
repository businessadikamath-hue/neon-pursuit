"""Original v8 scenery assets. Run in installed Blender; no imported assets."""
...
def trunk(kind,lod):
    ...
    h,ra,rb={'coastal':(4.08,.29,.165),'desert':(2.688,.20,.114),'kapok':(4.08,.42,.24),'conifer':(8.4,.23,.025),'palm':(8.32,.25,.12)}[kind]
    rings=18 if lod==1 else 32;sides=12 if lod==1 else 20
    verts=[];faces=[];uvs=[]
    for j in range(rings+1):
        t=j/rings;center=(.85*math.sin(t) if kind=='palm' else .18*t,h*t,.12*t if kind in ['kapok','coastal','desert'] else 0)
        for i in range(sides+1):
            a=i/sides*math.tau
            flare=(.07 if kind in ['palm','conifer'] else .15 if kind=='kapok' else .10)*math.exp(-t*12)
            ridge=(.018 if kind=='kapok' else .010)*math.sin(a*7+t*3.5)*(1-t*.5)
            ring=.010*math.sin(t*math.tau*13) if kind=='palm' else .003*math.sin(a*19+t*70)
    ...
    if kind!='palm':
        ...
        for k in range(roots):
            a=k/roots*math.tau+rng.uniform(-.12,.12);length=(1.15 if kind=='kapok' else .64 if kind=='coastal' else .48)*(1+rng.random()*.30)
            height=1.55 if kind=='kapok' else .68 if kind=='coastal' else .39;thick=.11 if kind=='kapok' else .060
            base=len(verts);steps=8 if lod==1 else 12;rs=6 if lod==1 else 8
            for j in range(steps+1):
                t=j/steps;rad=.06+length*t;yy=height*(1-t)**2+.025
                center=(math.sin(a)*rad,yy,math.cos(a)*rad);width=thick*(1-t*.92)
                for q in range(rs+1):
                    angle=q/rs*math.tau
                    verts.append((center[0]+math.cos(a)*math.sin(angle)*width,center[1]+math.cos(angle)*width*.74,center[2]-math.sin(a)*math.sin(angle)*width))
...
def record(obj):
    ...
    obj['source']='Original v8 Blender scenery; editable generation script included'
...
bpy.ops.wm.save_as_mainfile(filepath=os.path.join(OUT,'original-scenery-v8.blend'))
module='// Generated by work/scenery-blender8.py in Blender '+bpy.app.version_string+'. Original assets only.\nexport const blenderScenery8='+json.dumps(bank,separators=(',',':'))+';\n'
with open(os.path.join(ROOT,'..','..','source','blenderScenery8.js'),'w',encoding='utf-8') as f:f.write(module)
