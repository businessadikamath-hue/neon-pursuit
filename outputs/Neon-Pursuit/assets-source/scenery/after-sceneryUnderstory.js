...
import {routeBlockFor} from '../engine-v9/sceneryPolicy.js';
...
class PlantGeometry {
  ...
  blade(base,az,length,width,lift,droop,c,steps=7,heart=false,torn=false){
    ...
    for(let j=0;j<=steps;j++){
      ...
      const center=base.clone().addScaledVector(dir,length*t);center.addScaledVector(side,Math.sin(t*2.8+az)*width*.11*t);center.y+=lift*t-droop*t*t;
      const w=Math.max(.003,width*shape*(heart?(1.20-.40*t):1));
      const row=[];for(let k=0;k<3;k++){const s=k-1,p=center.clone().addScaledVector(side,s*w*(1+s*.055*Math.sin(az))*(torn&&j%3===1?.85:1));p.y-=Math.abs(s)*w*.17;p.y+=Math.sin(t*18+az)*w*.03;row.push(this.point(p,c.clone().multiplyScalar(.88+t*.17),k/2,t));}ids.push(row);
    }
...
function plant(kind,lod){
  const wood=new PlantGeometry(),leaf=new PlantGeometry(),r=rng(4727+kind*301),fine=lod===1,green=()=>new T.Color().setHSL(.24+r()*.045,.32+r()*.16,.25+r()*.12),stem=new T.Color(0x697947),bark=new T.Color(0x726248);
  const tube=(a,b,ra=.02,rb=.008,col=stem)=>wood.tube(a,b,ra,rb,col,fine?6:4);
  if(kind===0){
    const n=fine?8:6;for(let k=0;k<n;k++){const a=k*2.399+(r()-.5)*.35,len=.72+r()*.83,h=.36+r()*.42;let last=vec(0,.04,0);const count=fine?12:8;
    ...
    const n=kind===1?(fine?7:5):(fine?8:5);for(let k=0;k<n;k++){const a=k*2.399+(r()-.5)*.40,h=kind===1?.90+r()*1.4:.38+r()*.80,base=vec(Math.sin(a)*.13,.01,Math.cos(a)*.13),tip=vec(Math.sin(a)*(.24+r()*.12),h,Math.cos(a)*.31);tube(base,tip,kind===1?.054:.026,.014);leaf.blade(tip,a,kind===1?1.1+r()*.78:.68+r()*.60,(kind===1?.28:.44)*(.84+r()*.28),kind===1?.60:.35,kind===1?.90:.58,green(),fine?10:6,kind===2,kind===1);}
...
function resources(){
  ...
  const texture=plantSurface8('leaf').map;
