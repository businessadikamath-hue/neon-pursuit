...
import {blenderUltraScenery8} from '../engine-v9/blenderUltraScenery8.js';
import {scenerySurface8} from '../engine-v9/scenerySurface8.js';
import {plantSurface9 as plantSurface8} from '../engine-v9/plantSurface9.js';
import {anchorNearJunction,routeBlockFor} from '../engine-v9/sceneryPolicy.js';
...
function blades(biome,litter=false){
 ...
 const blade=(x,z,a,h,w,len,c,phase)=>{const base=p.length/3,steps=5;for(let j=0;j<=steps;j++){const t=j/steps,shape=Math.max(.018,Math.pow(Math.sin(t*Math.PI),.8)),along=len*(litter?t:t*t),bend=litter?Math.sin(t*Math.PI)*(.016+h)+Math.pow(t,4)*h:h*(t-Math.pow(t,4)*.16);for(let k=-1;k<=1;k++){const ww=k*w*(litter?shape:1-t*.95)*(1+k*.13*Math.sin(phase+t*4)),curl=litter?Math.abs(k)*w*(.18+Math.sin(t*4+phase)*.24):-Math.abs(k)*w*.16,y=bend+curl,shade=.84+t*.20;point(x+Math.sin(a)*along+Math.cos(a)*(ww+Math.sin(t*2.8+phase)*len*.10*t),y,z+Math.cos(a)*along-Math.sin(a)*ww,(k+1)/2,t,new T.Color(c).multiplyScalar(shade));}}for(let j=0;j<steps;j++)for(let k=0;k<2;k++){const n=base+j*3+k;idx.push(n,n+1,n+3,n+1,n+4,n+3);}};
 const n=litter?8:9;for(let k=0;k<n;k++){const a=k*2.399+(r()-.5)*1.1,c=new T.Color().setHSL((litter?.10:dry?.13:cold?.14:.245)+(r()-.5)*.028,litter?.20+r()*.1:dry?.26:cold?.13:.30+r()*.12,(litter?.22:dry?.39:cold?.47:.28)+(r()-.5)*.08);const h=(dry?.32:cold?.24:.58)*(.42+r()*1.04),x=Math.sin(a)*r()*(litter?.47:.19),z=Math.cos(a)*r()*(litter?.47:.19),len=litter?.11+r()*.19:h*(.24+r()*.40),phase=r()*6.28;blade(x,z,a,litter?.008+r()*.023:h,litter?.028+r()*.041:.005+r()*.010,len,c,phase);
  ...
  if(!litter&&(k===1||k===6))for(let seed=0;seed<5;seed++){const t=.68+seed*.046,xx=x+Math.sin(a)*len*t*t+Math.cos(a)*Math.sin(t*2.8+phase)*len*.10*t,zz=z+Math.cos(a)*len*t*t,yy=h*(t-Math.pow(t,4)*.16),b=p.length/3,c2=new T.Color(c).multiplyScalar(1.18),rad=.004+(1-Math.abs(seed-2)/3)*.003;point(xx-rad,yy,zz,0,0,c2);point(xx+rad,yy,zz,1,0,c2);point(xx,yy+.021,zz+rad,.5,1,c2);point(xx,yy+.009,zz-rad,.5,.5,c2);idx.push(b,b+1,b+2,b,b+3,b+1,b,b+2,b+3,b+1,b+3,b+2);}
 }
...
function resources(biome){
 ...
 const litter=new T.MeshStandardMaterial({vertexColors:true,side:T.DoubleSide,roughness:.99,...plantSurface8('leaf'),normalScale:new T.Vector2(.26,.26)}),grass=new T.MeshStandardMaterial({vertexColors:true,side:T.DoubleSide,roughness:.95,...plantSurface8('leaf'),normalScale:new T.Vector2(.11,.11)});
 ...
 function create(){
  ...
  for(const [ti,tile]of world.tiles.entries()){
   ...
   for(const spec of res.specs){const mesh=new T.InstancedMesh(spec.geometry,spec.material,spec.count),matrices=[];mesh.name=spec.name;mesh.userData.ultraScenery=true;mesh.castShadow=!spec.name.includes('grass');mesh.receiveShadow=true;mesh.count=0;mesh.visible=false;mesh.frustumCulled=false;
    for(let i=0;i<spec.count;i++){const side=r()<.5?1:-1,cluster=Math.floor(r()*9),phase=cluster*2.399+ti*.81+side*.62,centerZ=-9-cluster*17+Math.sin(phase)*5.4,z=T.MathUtils.clamp(centerZ+(r()+r()-1)*13,-156,-3),centerX=17.2+(.5+.5*Math.sin(phase*1.71))*4.3,x=spec.shore?-33-r()*8:side*Math.max(15.7,centerX+(r()-.5)*3.4),size=spec.scale+r()*spec.range;const h=tile.userData.terrainHeight?.(x,z)??-.1;dummy.position.set(x,h+.005,z);dummy.rotation.set(0,r()*Math.PI*2,0);dummy.scale.set(size*(.8+r()*.4),size*(.66+r()*.53),size);dummy.updateMatrix();matrices.push(dummy.matrix.clone());mesh.setColorAt(i,new T.Color().setScalar(.79+r()*.21));}
